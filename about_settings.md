...
上部の項目はコントロールパネルの`配信`タブで`顔・表情`に表示されているのとほぼ同じですが、設定ウィンドウの場合、`顔トラッキング中も自動でまばたき`チェックが指定できます。顔トラッキング時に目の開閉がうまく反映されず、見栄えに問題がある場合は、このチェックをオンにしてください。
...
キーボードの項目でも`配信`タブと同様、タイピング時のエフェクトを設定できます。

### 3.3.3. ゲームパッド

キーボード・マウスパッドと同じく、ゲームパッドの高さや大きさ、表示・非表示を設定できます。

※v0.9.0ではゲームパッドは常に非表示になっています。
