...
v0.9.2以降では、配信タブに"Word To Motion"メニューが入っています。この機能では色々な事ができますが、ここでは基本的に「表情をコントロールできる機能」として紹介します。詳しくは[3: 詳細設定](./about_settings.html)の「3.5. Word to Motion」を参照下さい。
...
**NOTE:** `この機能専用にデバイスを割り当て`でデバイスを選ぶと、そのデバイスを操作してもキャラクターは動かなくなります。次のように使い分けると有効です。

* `なし`を選ぶケース: 配信用途ではなく、単にキャラクターを置いておきたいとき。
    - `なし`にすると、キャラクターの動きと実際のあなたの動きが最も一致します。
* `ゲームパッド`を選ぶケース: ゲームよりも作業画面の配信、共有がしたいとき。
